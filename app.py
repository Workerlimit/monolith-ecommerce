...
class User(db.Model):
    ...
    cart_items = db.relationship('Product', secondary='cart', lazy='subquery', backref=db.backref('users', lazy=True))
...
def get_cart_items(user_id):
    ...
    for cart_item in user.cart_items:
        ...
        quantity = Order.query.filter_by(user_id=user_id, product_id=product['id']).first().quantity
        product['quantity'] = quantity
        cart_items.append(product)

    return jsonify({"cart_items": cart_items})

@app.route("/order/<user_id>/clear", methods=["POST"])
...
def add_to_cart(user_id):
    ...
    if cart_item is not None:
        ...
        cart_item = Order(user_id=user_id, product_id=product_id, quantity=quantity)
...
@app.route("/order/<user_id>/items/<product_id>", methods=["PUT"])
...
@app.route("/order/<user_id>/items/<product_id>", methods=["DELETE"])
...
@app.route("/order/<user_id>/update", methods=["POST"])
